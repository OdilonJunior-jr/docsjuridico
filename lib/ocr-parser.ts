...
const BAD_NAME = /REP[ÚU]BLICA|FEDERATIVA|BRASIL|CARTEIRA|NACIONAL|HABILITA[CÇ][AÃ]O|DRIVER|LICENSE|VALIDADE|EMISS[AÃ]O|IDENTIDADE|DOCUMENTO|ASSINATURA|FILIA[CÇ][AÃ]O|DETRAN|SECRETARIA|MINIST[EÉ]RIO|TRANSPORTES|CPF|NASCIMENTO|REGISTRO|CATEGORIA|PERMISS[AÃ]O|ENDERE[CÇ]O|UNIDADE|CONSUMIDORA|FATURA|ENERGIA|[ÁA]GUA|TELEFONE|CLIENTE/i
...
function parseAddress(lines: string[], text: string) {
  ...
  logradouro = findAfterLabel(lines, [/(?:ENDERECO|LOGRADOURO)/i], looksLikeAddress)
  if (!logradouro) {
  ...
  if (logradouro) {
    ...
    if (numberMatch && clean(numberMatch[1]).length >= 4) {
      ...
      if (tail && !/CEP/i.test(tail) && tail.length <= 60 && !/^(?:APTO|APT\.?|APARTAMENTO|BLOCO|BL\.?|CASA|LOTE|QD\.?|QUADRA|COMPLEMENTO|SALA|FUNDOS)\b/i.test(tail)) bairro = tail.replace(/^[-,]\s*/, '')
    }
  }
    ...
    if (cepIndex >= 0) {
      ...
      if (!bairro) {
        for (const candidate of around) {
          ...
          if (!/CPF|CNPJ|CLIENTE|CONTA|INSTALACAO|REFERENCIA|VENCIMENTO|TOTAL|ENERGIA|AGUA|TELEFONE|FATURA|ENDERECO|UNIDADE|CONSUMIDORA/.test(n) && candidate.length >= 3 && candidate.length <= 60 && !/^\d/.test(candidate)) {
